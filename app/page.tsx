    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="py-20 bg-charcoal-700">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-2">{t('home.featuredEvents')}</h2>
              <div className="h-1 w-20 bg-gold-500 rounded"></div>
            </div>
            <Link href="/events">
              <Button variant="ghost" className="text-gold-500 hover:text-gold-600">{t('home.viewAllEvents')} &rarr;</Button>
            </Link>
          </div>
              ...
              <Card key={event.id} className="overflow-hidden group border-charcoal-500 bg-white shadow-md hover:border-gold-500/50 transition-all duration-300">
                <div className="relative h-64 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4">
                    {event.isEarlyBird && (
                      <span className="bg-gold-500 text-black text-xs font-bold px-2 py-1 rounded-sm uppercase tracking-wider">{t('events.earlyBird')}</span>
                    )}
                    {event.isLastMinute && (
                      <span className="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-sm uppercase tracking-wider">{t('events.lastMinute')}</span>
                    )}
                  </div>
                </div>
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-sm">
                      <span className="text-gray-500">{t('home.startingFrom')} </span>
                      <span className="text-gold-600 font-bold text-lg">{event.basePrice.Standard} {t('common.qar')}</span>
                    </div>
      ...
      <section className="py-20 bg-gray-900">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold text-white mb-12">{t('home.venues')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="relative group overflow-hidden rounded-xl aspect-video">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-6">
                <h3 className="text-xl font-bold text-white">{t('home.operaHouse')}</h3>
              </div>
            </div>
            <div className="relative group overflow-hidden rounded-xl aspect-video">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-6">
                <h3 className="text-xl font-bold text-white">{t('home.kataraAmphitheatre')}</h3>
              </div>
            </div>
            <div className="relative group overflow-hidden rounded-xl aspect-video">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-6">
                <h3 className="text-xl font-bold text-white">{t('home.conventionCenter')}</h3>
              </div>
            </div>
          </div>
        </div>
