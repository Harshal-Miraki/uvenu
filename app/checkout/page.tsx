        ...
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
                <Card className="max-w-md w-full text-center bg-white border-charcoal-500 p-8 shadow-sm">
                    ...
                    <p className="text-gray-500 mb-6">Looks like you haven't added any tickets yet.</p>
                    <Link href="/events">
                                                    ...
                                                    <div className="flex justify-between items-start mb-3">
                                                        <div>
                                                            ...
                                                            <p className="text-sm text-gray-500">
                                                                {new Date(item.eventDate).toLocaleDateString()} • {item.seats.length} seat{item.seats.length > 1 ? 's' : ''}
                                                            </p>
                                                        </div>
                                                        <div className="flex flex-col items-end gap-2">
                                                            <span className="font-bold text-gray-900">{item.totalPrice} QAR</span>
                                                            <button
                                                                onClick={() => removeFromCart(item.eventId)}
                                                                className="text-xs text-red-500 hover:text-red-700 flex items-center"
                                                            >
                                                                <Trash2 className="w-3 h-3 mr-1" /> Remove All
                                                            </button>
                                                        </div>
                                                    </div>
                                        ...
                                        return (
                                            <div key={`${item.eventId}-${item.category}`} className="flex justify-between items-start pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                                                <div>
                                                    ...
                                                    <div className="text-sm text-gold-600 mt-1">{item.quantity} x {item.pricePerSeat} QAR</div>
                                                </div>
                                                <div className="flex flex-col items-end gap-2">
                                                    <span className="font-bold text-gray-900">{(item.quantity || 0) * (item.pricePerSeat || 0)} QAR</span>
                                                    <button
                            ...
                            <CardContent className="p-6">
                                ...
                                <div className="p-4 rounded border border-gold-500/30 bg-gold-500/5 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-gray-900 font-medium">Mock Payment Gateway</div>
                                            <div className="text-xs text-gray-500">No actual payment will be taken</div>
                                        </div>
                                    </div>
                        ...
                        <Card className="bg-white border-charcoal-500 sticky top-24 shadow-sm">
                            <CardContent className="p-6">
                                ...
                                <p className="text-xs text-center text-gray-400 mt-4 flex items-center justify-center">
                                    <span className="w-2 h-2 rounded-full bg-green-500 mr-2"></span> Secure Transaction
                                </p>
                            </CardContent>
                        </Card>
                    </div>
